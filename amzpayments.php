...
require_once(CURRENT_AMZ_MODULE_DIR . '/classes/AmazonPostalCodesHelper.php');
...
require_once(CURRENT_AMZ_MODULE_DIR . '/classes/AmazonPaymentsTroubleshooter.php');
require_once(CURRENT_AMZ_MODULE_DIR . '/classes/AmazonPaymentsCarts.php');
require_once(CURRENT_AMZ_MODULE_DIR . '/vendor/amazonpaycookie.php');
...
class AmzPayments extends PaymentModule
{
    ...
    public $button_enhancement_product = 0;
    ...
    public static $config_array = array(
        ...
        'button_enhancement_product' => 'AMZ_BUTTON_ENHANCEMENT_PRODUCT',
    ...
    public function __construct()
    {
        ...
        $this->cookie = new Cookie('psamzpay');

        if (isset($this->cookie->amz_access_token_set_time)) {
            if ($this->cookie->amz_access_token_set_time < time() - 3000) {
                unset($this->cookie->amz_access_token);
                unsetAmazonPayCookie();
            }
        }
        include_once(CURRENT_AMZ_MODULE_DIR . '/vendor/AmazonPayV2/autoload.php');
    }
    ...
    protected function resetDefaultSettings()
    {
        ...
        Configuration::updateValue('AMZ_BUTTON_ENHANCEMENT_PRODUCT', false);
        ...
        Configuration::updateValue('AMZ_NO_TOKEN_AJAX', '1');
    ...
    private function _postValidation()
    {
        if (Tools::isSubmit('submitAmzpaymentsModule') || Tools::isSubmit('submitAmzpaymentsModuleConnect')) {
            foreach (self::$config_array as $name => $f) {
                if (Tools::getValue($f) === false && !in_array($f, array('AMZ_FORCE_NAME_COMPLETION', 'AMZ_BUTTON_ENHANCEMENT_CART', 'AMZ_BUTTON_ENHANCEMENT_PRODUCT', 'SHIPPINGS_NOT_ALLOWED', 'PROVOCATION', 'PRODUCTS_NOT_ALLOWED', 'AMZ_HIDE_LOGIN_BTNS', 'AMZ_HIDE_MINICART_BUTTON', 'AMZ_PROMO_HEADER', 'AMZ_PRODUCT_PAGE_CHECKOUT', 'AMZ_CARRIERS_MAPPING'))) {
        ...
        $button_enhancement_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Amazon Pay button enhancement on the product page'),
                        'name' => 'AMZ_BUTTON_ENHANCEMENT_PRODUCT',
                        'is_bool' => true,
                        'values' => array(
                            array(
                                'id' => 'active_on_button_enhancement_product',
                                'value' => true,
                                'label' => $this->l('Enabled')
                            ),
                            array(
                                'id' => 'active_off_button_enhancement_product',
                                'value' => '0',
                                'label' => $this->l('Disabled')
                            )
                        )
    ...
    public function getContent()
    {
        ...
        if (Tools::getValue('getPublicKey') == 'true') {
            ...
            header("Content-Length: " . filesize(CURRENT_AMZ_MODULE_DIR . '/keys/public.key'));
            readfile(CURRENT_AMZ_MODULE_DIR . '/keys/public.key');
            exit();
        }
        ...
        $this->context->smarty->assign('pubkey_link', file_exists(CURRENT_AMZ_MODULE_DIR . '/keys/public.key') ? true : false);
        $this->context->smarty->assign('alexa_region', $this->getRegionMappingForSDKv2());
        $this->context->smarty->assign('alexa_public_key', $this->getPublicKeyString(CURRENT_AMZ_MODULE_DIR . '/keys/public.key'));
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        $this->context->controller->addJS(($this->_path) . 'views/js/admin_17.js');
        $this->context->controller->addCSS(($this->_path) . 'views/css/admin_17.css');
    }
    ...
    protected function generateKeypair()
    {
        try {
            ...
            $keydir = CURRENT_AMZ_MODULE_DIR . '/keys';
            if (!is_dir($keydir)) {
                mkdir($keydir);
            }
    ...
    {
        $csv = array_map('str_getcsv', file(CURRENT_AMZ_MODULE_DIR . '/vendor/amazon-pay-delivery-tracker-supported-carriers.csv'));
    ...
    public function hookDisplayExpressCheckout($params)
    {
        ...
        if ($show_amazon_button) {
            ...
            $this->context->smarty->assign('create_account', $create_account);
    ...
    public function hookDisplayProductButtons($params)
    {
        ...
        if ($show_amazon_button && $this->product_page_checkout == '1') {
            ...
            $this->context->smarty->assign('btn_url', $this->getButtonURL());
            $this->context->smarty->assign('hide_button', $this->button_visibility == '0');
            $this->context->smarty->assign('buttonEnhancement', $this->button_enhancement_product == '1');
    ...
    public function hookDisplayHeader($params)
    {
        ...
        $is_logged = 'false';
        if ((isset($this->cookie->amz_access_token) && $this->cookie->amz_access_token != '') || getAmazonPayCookie()) {
            $is_logged = 'true';
        }
        ...
        if ((isset($this->cookie->amz_access_token) && $this->cookie->amz_access_token != '') || getAmazonPayCookie()) {
            if (!isset($this->context->cookie->amazon_id)) {
                if (getAmazonPayCookie()) {
                    ...
                    $acc_tk = self::prepareCookieValueForAmazonPaymentsUse($this->cookie->amz_access_token);
                }
            }
        }
        ...
        return $css_string . $amz_login_ready . $ext_js . '<script src="https://code.jquery.com/jquery-1.12.4.min.js" integrity="sha256-ZosEbRLbNQzLpnKIkEdrPv7lOy9C27hHQ+Xp8a4MxAQ=" crossorigin="anonymous"></script><script type="text/javascript">' . $js_file . ' </script>' . $logout_str;
    }
    ...
    public function recreateAmzJsString()
    {
        ...
        if (getAmazonPayCookie() || (isset($this->cookie->amz_access_token) && $this->cookie->amz_access_token != '')) {
            if (!isset($this->context->cookie->amazon_id)) {
                if (getAmazonPayCookie()) {
    ...
    public function hookDisplayBanner($params)
    {
        ...
        if (Configuration::get('AMZ_PROMO_HEADER') == '1') {
            ...
            $content.= $this->display(__FILE__, 'views/templates/hooks/display_banner.tpl');
        }
        return $content;
    }
    ...
    public function setOrderReferenceAtAmazonPay($order, $amazon_order_reference_id, $total, $currency_code)
    {
        return;
        // prepared method for later use when activated at Amazon Pay
        if (!is_object($order)) {
        ...
        $requestParameters['seller_order_id'] = $order->reference;
        $requestParameters['amount'] = $total;
        $requestParameters['currency_code'] = $currency_code;
        $response = $service->setOrderAttributes($requestParameters);
    }
