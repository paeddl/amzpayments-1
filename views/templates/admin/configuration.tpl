...
{capture name=public_key_mail_assign assign=public_key_mail}{l s='Please [1]contact[/1] Amazon Pay to receive the Public Key ID' tags=['<a href="JavaScript:void(0)" id="public_key_mail_init">'] mod='amzpayments'}{/capture}
				...
				<div class="form-group">
					...
					<div class="col-lg-9">
						{foreach from=$amz_carrier_options item=co}
							<div class="row">
								<label class="col-lg-3">{$co.label|escape:'htmlall':'UTF-8'}:</label>
								<select name="AMZ_CARRIERS_MAPPING[{$co.val|escape:'htmlall':'UTF-8'}]" class="col-lg-9">
									<option value="">---</option>
									{foreach from=$amazon_carriers item=ac}
										<option value="{$ac[1]|escape:'htmlall':'UTF-8'}"{if isset($mapped_carriers[$co.val]) && $mapped_carriers[$co.val] == $ac[1]} selected{/if}>{$ac[0]|escape:'htmlall':'UTF-8'}</option>
									{/foreach}
								</select>
							</div>
						{/foreach}
